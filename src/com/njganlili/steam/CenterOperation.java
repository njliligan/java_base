...
public class CenterOperation {
    ...
    public static void main(String[] args){
        ...
        List<User> userList =users.stream()
                //条件guolv
                .filter(user -> {return "M".equals(user.getSex());})
                //去重,使用hash和equals
                .distinct()
                //跳过n个元素
                .skip(1)
        ...
        List<User> userPeeks = users.stream().peek(user -> {user.setAge(100);}).collect(Collectors.toList());
    }
